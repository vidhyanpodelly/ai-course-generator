...
2. The following variables MUST be configured:
   ...
   - **`AI_BASE_URL`**: The public URL where your OmniRoute Docker service is hosted (e.g., `https://omniroute-service.onrender.com/v1`). If not set, the app will throw a fatal error on startup.
...
2. Point it to the `omniroute/` directory in this repository.
3. Use the provided `omniroute/render.yaml` or configure manually using `omniroute/Dockerfile`.
4. Add the `GEMINI_API_KEY` and `GROQ_API_KEY` environment variables.
5. Provide a persistent disk mounted at `/app/data` to save combos and rules.
6. Note the public URL generated by Render (e.g., `https://your-omniroute.onrender.com`) and update `AI_BASE_URL` in the backend.
...
1. **Check Backend Logs**: During Spring Boot startup, the `AIHealthCheckService` attempts to ping `${AI_BASE_URL}/models`. Look for `AI Health Check Failed` in the logs.
2. **Verify AI_BASE_URL**: Ensure the `AI_BASE_URL` environment variable is set to the correct public URL of your deployed OmniRoute Docker instance (e.g., `https://your-omniroute.onrender.com/v1`). It must include `/v1`.
3. **Check OmniRoute Deployment**: Ensure the OmniRoute Docker service is actively running on Render and hasn't crashed due to missing API keys (e.g., `GEMINI_API_KEY`, `GROQ_API_KEY`).
4. **Test Health Endpoint**: Manually navigate to `https://your-backend.onrender.com/api/health/ai` in your browser. It will return `{"status": "UP"}` if the AI is successfully connected, or `{"status": "DOWN", "reason": "..."}` detailing why it failed.
