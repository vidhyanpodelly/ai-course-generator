...
5. **Lazy Load Lessons**: Expand a chapter and click **Study** on any lesson. OpenRouter processes the contextual matches and generates rich markdown details.
...
1. **Check Backend Logs**: During Spring Boot startup, the `AIHealthCheckService` attempts to ping the OpenRouter API. Look for `AI Health Check Failed` in the logs.
2. **Verify OPENROUTER_API_KEY**: Ensure the `OPENROUTER_API_KEY` environment variable is set correctly in your backend environment.
